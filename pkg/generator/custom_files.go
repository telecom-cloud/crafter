...
func (pkgGen *HttpPackageGenerator) genCustomizedFile(pkg *PackageDescription) error {
	filePathRenderInfo := FilePathRenderInfo{
		MasterIDLName: pkg.IdlName,
		GenPackage:    pkg.Package,
		ModelDir:      pkgGen.ModelDir,
		ProjectDir:    pkgGen.TemplateGenerator.OutputDir,
	...
	for _, tplInfo := range pkgGen.TemplateGenerator.Templates {
		// the default template has been automatically generated by the tool, so skip
		if tplInfo.Info.Default {
			continue
		}

		// loop generate file
		if tplInfo.Info.LoopService || tplInfo.Info.LoopMethod {
			loopMethod := tplInfo.Info.LoopMethod
			loopService := tplInfo.Info.LoopService
			if loopService && !loopMethod { // only loop service
				for _, service := range idlPackageRenderInfo.ServiceInfos.Services {
					...
					err := pkgGen.genLoopService(&tplInfo.Info, filePathRenderInfo, service, &idlPackageRenderInfo)
					if err != nil {
						return err
					}
				}
				...
				for _, service := range idlPackageRenderInfo.ServiceInfos.Services {
					for _, method := range service.Methods {
						...
						err := pkgGen.genLoopMethod(&tplInfo.Info, filePathRenderInfo, method, service, &idlPackageRenderInfo)
						if err != nil {
							return err
						}
					}
			...
			err := pkgGen.genSingleCustomizedFile(&tplInfo.Info, filePathRenderInfo, idlPackageRenderInfo)
			if err != nil {
				return err
			}
		}
...
func renderFilePath(tplInfo *tpl.Template, filePathRenderInfo FilePathRenderInfo) (string, error) {
	tpl, err := template.New(tplInfo.Path).Funcs(tpl.FuncMap).Parse(tplInfo.Path)
	if err != nil {
...
func renderInsertKey(tplInfo *tpl.Template, data interface{}) (string, error) {
	tpl, err := template.New(tplInfo.UpdateBehavior.InsertKey).Funcs(tpl.FuncMap).Parse(tplInfo.UpdateBehavior.InsertKey)
	if err != nil {
...
func renderImportTpl(tplInfo *tpl.Template, data interface{}) ([]string, error) {
	var importList []string
	for _, impt := range tplInfo.UpdateBehavior.ImportTpl {
		tpl, err := template.New(impt).Funcs(tpl.FuncMap).Parse(impt)
		if err != nil {
...
func renderAppendContent(tplInfo *tpl.Template, renderInfo interface{}) (string, error) {
	tpl, err := template.New(tplInfo.Path).Funcs(tpl.FuncMap).Parse(tplInfo.UpdateBehavior.AppendTpl)
	if err != nil {
...
func appendUpdateFile(tplInfo *tpl.Template, renderInfo interface{}, fileContent []byte) ([]byte, error) {
...
func getInsertImportContent(tplInfo *tpl.Template, renderInfo interface{}, fileContent []byte) ([][2]string, error) {
...
func (pkgGen *HttpPackageGenerator) genLoopService(tplInfo *tpl.Template, filePathRenderInfo FilePathRenderInfo, service *Service, idlPackageRenderInfo *IDLPackageRenderInfo) error {
	...
	if !exist { // create file
		...
		case tpl.Append: // todo: append logic need to be optimized for method
			...
			if tplInfo.UpdateBehavior.AppendKey == "method" {
				...
				if len(tplInfo.UpdateBehavior.AppendLocation) == 0 { // default, append to end of file
					...
					pkgGen.SetFiles(append(pkgGen.Files(), util.File{Path: filePath, Content: buf.String()}))
...
func (pkgGen *HttpPackageGenerator) genLoopMethod(tplInfo *tpl.Template, filePathRenderInfo FilePathRenderInfo, method *HttpMethod, service *Service, idlPackageRenderInfo *IDLPackageRenderInfo) error {
	filePath, err := renderFilePath(tplInfo, filePathRenderInfo)
	if err != nil {
		return err
	}
...
func (pkgGen *HttpPackageGenerator) genSingleCustomizedFile(tplInfo *tpl.Template, filePathRenderInfo FilePathRenderInfo, idlPackageRenderInfo IDLPackageRenderInfo) error {
	...
	if !exist { // create file
		...
		case tpl.Append: // todo: append logic need to be optimized for single file
			fileContent, err := ioutil.ReadFile(filePath)
			if err != nil {
				return err
			}
			if tplInfo.UpdateBehavior.AppendKey == "method" {
				...
				if len(tplInfo.UpdateBehavior.AppendLocation) == 0 { // default, append to end of file
					...
					logs.Infof("append content for file '%s', because the update behavior is 'Append' and appendKey is 'method'", filePath)
					pkgGen.SetFiles(append(pkgGen.Files(), util.File{filePath, buf.String(), false, ""}))
				}
				...
				pkgGen.SetFiles(append(pkgGen.Files(), util.File{Path: filePath, Content: string(file)}))
			}
		default:
			// do nothing
			logs.Warnf("unknown update behavior, do nothing")
